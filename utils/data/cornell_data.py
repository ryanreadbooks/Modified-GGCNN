...
class CornellDataset(GraspDatasetBase):
    ...
    """
    def __init__(self, file_path=None, start=0.0, end=1.0, ds_rotate=0, **kwargs):
        """
        ...
        if file_path is not None:
            ...
            depthf = [f.replace('cpos.txt', 'd.tiff').replace('pos_label', 'depth_image') for f in graspf]
            rgbf = [f.replace('d.tiff', 'r.png').replace('depth_image', 'image') for f in depthf]

            self.grasp_files = graspf[int(l*start):int(l*end)]
            self.depth_files = depthf[int(l*start):int(l*end)]
            self.rgb_files = rgbf[int(l*start):int(l*end)]

            logging.info('Cornell dataset created!!')
